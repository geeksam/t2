class DashboardController < ApplicationController
  ...
  def index
    build_time_summaries
    @current_tb   = TimeBlock.current
    @current_task = @current_tb.try(:task)

    @todays_tbs = TimeBlock.for_most_recent_date.all
  end
  ...
  def edit_todays_tbs
    build_time_summaries
    @tbs = TimeBlock.for_most_recent_date.all
...
protected
  ...
  def build_time_summaries
    weeks = [Week.for_date(Date.today)]
    (1..3).each do |i|
      weeks << weeks.last.prev
    end
    @time_summaries = {
      :today  => sum_time(:for_day,        TimeBlock.maximum('date')),
